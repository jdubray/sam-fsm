...
function stateMachineAcceptors({ pc0, actions, states, composite, transitions, pc, id, componentName, deterministic, lax, enforceAllowedTransitions }) {
    const specification = getStatesFrom(states, transitions)
    const stateLabels = Object.keys(specification.states)
    actions = actions || specification.actions
    const acceptors = deterministic 
        ? [model => proposal => {
            if (proposal.__stateMachineId === id) {
                const currentState = modelGetValue(model, componentName, pc)
...
function fsm ({ id = Date.now(), componentName, pc0, actions, transitions, states, composite, pc = 'pc', deterministic = false, lax = true, enforceAllowedTransitions = false, blockUnexpectedActions = false }) {
    return {
        id,
        initialState: model => { 
        ...
        stateMachine: stateMachineReactor({ id, componentName, pc0, actions, states, composite, transitions, pc, deterministic, lax, enforceAllowedTransitions, blockUnexpectedActions }),
        acceptors: stateMachineAcceptors({ id, componentName, pc0, actions, states, composite, transitions, pc, deterministic, lax, enforceAllowedTransitions }),
