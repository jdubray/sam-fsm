...
function stateMachineReactor({ pc0, actions, transitions, states, pc = 'pc', componentName, deterministic = false, lax = true, enforceAllowedTransitions = false, blockUnexpectedActions = false }) {
    ...
    if (blockUnexpectedActions) {
        smr.push( model => () => {
            ...
            if (model.__allowedActions.length === 0) {
...
const gvt = (start, end, action = '') => `${start} -> ${end} [label = "${action}"];`
...
function renderGraphViz({ pc0, actions, states, transitions, deterministic }) {
    ...
    const graphVizTransitions = Object.keys(states).map(state => {
        ...
        return (states[state].transitions || []).map(transition => gvt(state, actions[transition][0], transition)).join('\n')
    }).join('\n')
