...
const updateRuntime = (stateMachine, currentState, previousState, action) => {
    if (action) {
        ...
        if (!stateMachine.actions[action]) {
            stateMachine.actions[action] = [currentState]
        } else {
            stateMachine.actions[action].indexOf(currentState) === -1
            && stateMachine.actions[action].push(currentState)
        }
        if (previousState) {
            if (stateMachine.states[previousState]) {
                stateMachine.states[previousState].transitions.indexOf(action) === -1
                && stateMachine.states[previousState].transitions.push(action)
            } 
        }
    }
}
...
function fsm ({ id = Date.now(), componentName, pc0, actions, transitions, states, composite, pc = 'pc', deterministic = false, lax = true, enforceAllowedTransitions = false, blockUnexpectedActions = false , stateDiagram = {} }) {
    return {
        id,
        initialState: model => { 
